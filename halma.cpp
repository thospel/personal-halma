...
BoardSubsetRedBuilder::BoardSubsetRedBuilder(uint t, size_t size) :
    ...
    mallocate(armies_, allocated());
    armies_end_ = armies_ + allocated();
...
BoardSubsetRedBuilder::~BoardSubsetRedBuilder() {
    if (armies_end_)
        demallocate(armies_end_ - allocated(), allocated());
    if (fd_ >= 0) {
...
BoardSubsetRed BoardSubsetRedBuilder::extract(Statistics& stats) {
    auto sz = size();
    stats.boardset_try(sz);
    auto end = armies_;
    auto from = end - sz;
    lib_sort(from, end);
    ...
    if (red_file) {
        ...
        auto offset = offset_;
        offset -= b;
        for (ArmyId previous = 0; from < end; ++from)
            if (*from != previous) {
                previous = *from;
                base[b++] = previous;
                if (b == BLOCK) {
                    Write(fd_, base, b * sizeof(*base), filename_);
                    offset += b;
                    b = 0;
                }
            }
        ...
        offset_ = offset;
        return BoardSubsetRed(old_offset, sz);
    } else {
        armies_ = from;
        auto to = from;
        for (ArmyId previous = 0; from < end; ++from)
...
void BoardSubsetRedBuilder::resize() {
    ...
    auto base = armies_end_ - old_allocated;
    auto new_allocated = old_allocated * 2;
    remallocate(base, old_allocated, new_allocated);
    armies_end_ = base + new_allocated;
    armies_ = base + begin_ + sz;
...
size_t BoardSubsetRedBuilder::memory_report(ostream& os, string const& prefix) const {
    ...
    os << prefix << "armies: ";
    if (armies_end_) {
        os << "ArmyId[" << allocated() << "] (" << allocated() * sizeof(armies_[0]) << " bytes)\n";
        sz += allocated() * sizeof(armies_[0]);
    } else  os << "nullptr\n";
    return sz;
}
...
void BoardSetBlue::insert(Board const& board, ArmySet& armies_blue, ArmySet& armies_red) {
    ...
    pre_write(1);
    insert(blue_id, red_id, symmetry);
    post_write();
}
...
bool BoardSubsetRed::_insert(ArmyId red_value) {
...
bool BoardSetRed::_insert(ArmyId blue_id, ArmyId red_id, int symmetry) {
    if (CHECK) {
    ...
    bool result = at(blue_id)._insert(red_id, symmetry);
    size_ += result;
    return result;
}
...
void BoardSetRed::insert(Board const& board, ArmySet& armies_blue, ArmySet& armies_red) {
    ...
    subset_to.insert(red_id, symmetry);
    if (STATISTICS) dummy_stats.subset_size(subset_to.size());
    insert(blue_id, subset_to, dummy_stats);
    post_write();
}
...
void my_main(int UNUSED argc, char const* const* argv) {
    GetOpt options("b:B:t:IsHSjpqQ:eEFf:vV:R:Ax:y:r:a:c:LiT", argv);
    long long int val;
    bool replay = false;
